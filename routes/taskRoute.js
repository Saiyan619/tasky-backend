...
router.get("/taskInfo/:id", async (req, res) => {
  try {
    ...
    const collaboratorIds = singleTask.collaborators.map(
      (collab) => collab.clerkId
    );
    ...
    const populatedActivityLogs = await Promise.all(
      singleTask.activityLogs.map(async (log) => {
        const user = await User.findOne({ clerkId: log.userId }).select(
          "email"
        );
        return {
          action: log.action,
  ...
  try {
    ...
    if (task.userId === userId) {
      ...
      const updatedCollaborators = collaborators
        ? [
            ...new Map(
              [...task.collaborators, ...collaborators].map((c) => [
                c.clerkId,
                c,
              ])
            ).values(),
    ...
    if (!collaborator) {
      ...
      return res
        .status(403)
        .json({ message: "You are not a collaborator on this task" });
    }
    ...
    const updatedCollaborators = collaborators
      ? [
          ...new Map(
            [...task.collaborators, ...collaborators].map((c) => [c.clerkId, c])
          ).values(),
        ] // Merge by `clerkId`
      : task.collaborators;
...
router.post("/activityLog/:id", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Activity log added", activityLogs: task.activityLogs });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
